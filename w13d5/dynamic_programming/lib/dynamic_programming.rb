...
class DPProblems
  ...
  def fibonacci(n)
    return @fibres[n] if @fibres.include?(n)
    @fibres[n] = fibonacci(n - 2) + fibonacci(n - 1)
  end
  ...
  def make_change(amt, coins, coin_cache = {0 => 0})
    ...
    coins.each do |coin|
      soln = coin_cache[amt]
      soln = make_change(amt - coin, coins, coin_cache) if soln.nil?
      if !soln.nil?
        coin_cache[amt] = soln
        results << soln
      end
    end
    results.empty? ? nil : results.min
  end
